...
async def process_promo_code_input(message: types.Message, state: FSMContext,
    ...
    if is_suspicious:
        ...
        if success:
            ...
            # Check if this was a discount promo
            is_discount_promo = isinstance(result, str) and result.startswith("discount:")
            if is_discount_promo:
                discount_pct = int(result.split(":")[1])
                response_to_user_text = _(
                    "promo_code_discount_applied_success",
